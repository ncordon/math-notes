...
#+begin_src octave :session :results none :exports both :tangle ejercicio2-approx.m :eval yes
...
#+begin_src octave :session :results file :exports both :eval yes
% plot approximation
figure(1, "visible", "off" )
clf
...
#+begin_src octave :session :results file :exports both :eval never :tangle ejercicio2-error.m :eval yes
...
Puede observarse como al hacer la representación de las aproximaciones frente al verdadereo valor de la $u$ encontramos que a medida que evolucionamos en tiempo ambas convergen a la función constantemente $0$ (eje de abscisas).
...
for n = 1:nsteps
  ...
  plot(x, u, 'b.-', xfine, ufine, 'r', sleep(0.8))
...
\begin{align*}
\frac{3f(a) - 4f(a-h) + f(a -2h)}{2h} &= \frac{3f(a) - 4f(a) + 4f'(a)h + o(h^2) + f(a) -f'(a)2h + o(h^2)}{2h} =\\
&= \frac{2hf'(a)}{2h} = f'(a) + \frac{o(h^2)}{2h} \underset{h\rightarrow 0}{\longrightarrow} f'(a)
\end{align*}

\begin{align*}
\frac{f(a) - 2f(a-h) + f(a -2h)}{h^2} &= \frac{f(a) - 2f(a) + 2f'(a)h - \frac{2}{2}f''(a)h^2 + o(h^3) + f(a) -f'(a)2h + \frac{f''(a)}{2}4h^2 + o(h^3)}{h^2} =\\
&= f''(a) + \frac{o(h^3)}{h^2} \underset{h\rightarrow 0}{\longrightarrow} f''(a)
\end{align*}
...
\[
  u(t+k, x) \approx  u(t,x) + ku_t(t,x) + \frac{k^2}{2}u_{tt} + o(h^3)
\]
...
function ans = advectionPeriodicLW(N, a, ax, bx, initCond, f)
  % maximum time
  tfinal = 1;
...
ans = advectionPeriodicLW(N, a, ax, bx, initCond, f)
#+end_src
