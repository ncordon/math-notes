...
$\frac{u(t_n+h) - 2u(t_n) + u(t_n -h)}{h^2} - u(t_n) - f(t_n) = \frac{u((n+1)h) - 2u(nh) + u((n-1)h)}{h^2} - u(nh) - f(nh) = \tau_n$
...
\[\lim_{h\rightarrow 0} \frac{(n+1)^2 u''((n+1)h) - 2n^2 u''(nh) + (n-1)^2 u''((n-1)h)}{2} = \frac{(n+1)^2 - 2n^2 + (n-1)^2}{2} u''(0) = u''(0)\]
...
Y para $p$ arbitrario: 
\begin{align*}
\lim_{h\rightarrow 0} \frac{1}{\lambda_p} &= \lim_{h\rightarrow 0}\frac{h^2}{2cos(2\pi p h) - 2 + h^2} \underbrace{=}_{\textrm{L'Hôpital 2 veces}}\\
