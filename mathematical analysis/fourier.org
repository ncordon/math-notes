...
#+begin_fact
En $L^2([-\pi, \pi], \mathbb{C})$ tomamos el conjunto $B = \{x\mapsto e^{inx} : n\in \mathbb{Z}\}$ que es ortogonal. Veámoslo.
#+end_fact

#+begin_proof
En el espacio de Hilbert $L^2([-\pi,\pi], \mathbb{C})$ el producto escalar que tomamos es:
