...
Sea $R$ anillo no trivial, $\alpha\le R, \alpha\neq R$. Entonces $\exists J\supseteq \alpha$ ideal maximal de $R$. Así, todo anillo tiene al menos un ideal maximal.
#+end_corollary

#+begin_proof
Aplicando el teorema anterior al caso $S=\{1\}$. El ideal $\mathcal{M}$ que obtenemos es maximal verificando que $1\notin \mathcal{M}$ y que $\alpha \subseteq \mathcal{M}$, luego es maximal el $R$.
...
#+begin_lemma
Si $\mathcal{F}_1 \subset \mathcal{F}_2$ entonces $\mathbb{V} (\mathcal{F}_2) \supset \mathbb{V} (\mathcal{F}_1)$
#+end_lemma

#+begin_fact
Sea $\mathcal{F} \subseteq K[X_1, \ldots X_n]$ y sea $J = (\mathcal{F})$ el ideal generado por $\mathcal{F}$. Entonces:
...
2. Sean $J_1, J_2 \subseteq K[X_1, \ldots X_n]$. Entonces $\mathbb{V}( J_1 J_2 ) = \mathbb{V} (J_1) \cup \mathbb{V} (J_2) = \mathbb{V} (J_1 \cap J_2)$
...
#+begin_corollary
Sea $K$ cuerpo algebraicamente cerrado. Entonces existe una correspondencia biyectiva entre el retículo de las variedades algebraicas afines de $\mathbb{A}^n (K)$ y el retículo de ideales de polinomios que son radicales.
...
#+begin_fact
Sea $V \subseteq \mathbb{A}^n (K), W\in \subseteq \mathbb{A}^m (K)$ variedades y un homomorfismo de $K$ álgebras $h: K[W] \rightarrow K[V]$.

Supongamos $h(Y_j + \mathbb{I}(W)) = F_j + \mathbb{I}(V)$

Sea $A= K[X_1, \ldots X_n, Y_1, \ldots Y_m]$ y sea $C$ el ideal de $A$ generado por $\{Y_i-F_i\} \cup \mathbb{I}$

1. $Ker(h) = C\cap K[Y_1, \ldots Y_m] / \mathbb{I}(W)$
2. Dada $\mathbb{G}$ base de Groebner reducida de $C$ con orden lexicográfico $X_1 > \ldots X_n > Y_1 > \ldots Y_m$ Entonces:

\[F + \mathbb{I}(V) \in Img(h) \Leftrightarrow R(F,\mathbb{G}) \in K[Y_1, \ldots Y_m]\]

Y en tal caso: $F + \mathbb{I}(V) = h(R(F, \mathbb{G}) + \mathbb{I}(W))$
#+end_fact


#+begin_corollary
Para el homomorfismo de $K$ álgebras de la proposición anterior se cumple:

1. $h$ es inyectiva sii $C\cap K[Y_1, \ldots Y_m] \subseteq \mathbb{I}(W)$
2. $h$ es sobreyectiva sii para cada $i=1, \ldots n$ existe $M_i \in K[Y_1, \ldots Y_m]$ tal que $X_i-M_i \in \mathbb{G}$
#+end_corollary
...
Entonces $F = \sum_{i=1}^t Q_i G_i + R'$ donde $Q_j = Q_j', j\neq i$ y $Q_i = Q_i' + \frac{cl(F)}{cl(H)} X^{\gamma}$

Por tanto sólo nos queda comprobar que se cumple la hipótesis ii. para $Q_i$.

$exp(G_i) + N(Q_i) = exp(G_i) + (N(Q_i')\cup \{\gamma\}) = \Big(exp(G_i) + N(Q_i')\Big) \cup \Big(exp(G_i) + \gamma \Big) \subseteq \Delta^i$

En el caso b. tomo $F_1 = F - tl(F)$ y por hipótesis de inducción:

$F_1  =\sum_{i=1}^t Q_i' G_i + R'$ donde $\{Q_1', \ldots Q_t'\}$ verifican las hipótesis ii. y iii. del teorema. Así $F = \sum_{i=1}^t Q_i'G_i + R' + tl(F)$ donde $N(R' + tl(F)) \subseteq N(R') \cup \{exp(F)\} \subseteq \bar{\Delta}$

Veamos la unicidad. Supongamos existen $Q_i, Q_i', R, R'$ verificando las hipótesis. Restando las dos expresiones de $F$ tendríamos:

$0 = \sum_{i=1}^t (Q_i - Q_i') G_i + (R-R') \Leftrightarrow R'-R = \sum_{i=1}^t (Q_i - Q_i') G_i + R$. Si alguno de los $(Q_i - Q_i')$ fuese no nulos tendríamos $exp(R'-R) \subseteq \cup_{i=1}^t \Delta^i$ (estamos usando que los $\Delta^i$ son disjuntos y $exp(G_i) + N(Q_i - Q_i') \subseteq exp(G_i) + N(Q_i)\cup N(Q_i') \subseteq \Delta^i$ y por ello $exp \left(\sum_{i=1}^t (Q_i - Q_i')G_i \right) = max\{exp(G_i) + exp(Q_i-Q_i')\}$. Pero por otro lado $N(R'-R) \subseteq \bar{\Delta}$ y esto entra en contradicción con $exp(R') \subseteq \cup_{i=1}^t \Delta^i$
#+end_proof
*** Bases de Groebner
...
#+begin_wording
Sea $\le$ un orden en $\mathbb{N}^n$ que es total y compatible. Haciendo uso de la teoría de ideales monomiales, probad que $\le$ es un buen orden sii es monótono.
#+end_wording
...
3. $I = (X=A_1, Y^2 Z = A_2, YZ^2=A_3)$, y por otro lado $J=(X^3 YZ = B_1, X^2Y = B_2, Y^2 Z^3 = B_3)$
...
*** Ejercicio 3.5
#+begin_wording
Haciendo uso del teorema de los ceros de Hilbert, demostrad que para $J$ un ideal de $K[X_1, \ldots X_n]$ se tiene:

\[J \subsetneq K[X_1, \ldots X_n] \Leftrightarrow \mathbb{V}(J) \neq \{0\}\]

con $K$ algebraicamente cerrado.
#+end_wording

Si $J = K[X_1, \ldots X_n]$ entonces $\mathbb{V}(J) = \{0\}$ claramente.

Supongamos $\mathbb{V}(J) = \{0\}$. Entonces $J \subseteq \sqrt{J} = \mathbb{I} \mathbb{V} (J) = \mathbb{I}(\{0\}) = K[X_1, \ldots X_n]$, lo que no puede ocurrir si $\sqrt{J}$ fuese intersección de maximales propiamente contenidos en $R$, luego $J=K[X_1, \ldots X_n]$.

*** TODO Ejercicio 3.7
#+begin_wording
Sea $V$ una variedad algebraica, para cada $x\in V$ se considera $\mathcal{M}_x = \{f\in K[V] : f(x) = 0\}$. Se tiene:

1. $\matcal{M}_x$ es un ideal maximal de $K[V]$.
2. La aplicación $\lambda : V \rightarrow Max(K[V]), \lambda(x) = \mathcal{M}_x$ es una aplicación inyectiva.
3. La aplicación $\lambda$ es una aplicación sobreyectiva si $K$ es algebraicamente cerrado.
#+end_wording

*** Ejercicio 3.22
#+begin_wording
Se considera $K = \mathbb{C}$ el conjunto algebraico $V = \{(1,2), (1,3), (1,4)\}$ y  el conjunto algebraico $W = \mathbb{V}((Y_1 - 1)^2)$ y la aplicación $f: V\rightarrow W$ definida por: 

\[f(1,2) = (1,4,3), f(1,3) = (1,9,4), f(1,4) = (1,16,5)\]

1. Calcula el homomorfismo $\bar{f} : K[W] \rightarrow K[V]$
2. Calcular el núcleo de $\bar{f}$.
3. Calcular la imagen de $\bar{f}$.
4. Se considera la aplicación $g: K[V] \rightarrow K[W]$ definida por $g(X_1) = Y_1, g(X_2) = Y_3 - Y_1$. ¿Está $g$ bien definida?
#+end_wording

1. 
Claramente $W = \{(1, y, z): x,z \in K\}$ y $f(X,Y) = (X, Y^2, X+Y)$

LLamamos $F_1 = X, F_2 = Y^2, F_3 = X+Y$

$\bar{f}$ viene dada por $\bar{f}(Y_j + \mathbb{I}(W)) = F_i + \mathbb{I}(V)$


2. 

\begin{align*}
\mathbb{I}(V) = (X-1)^2 + (Y-2)^2 (Y-3)^2 (Y-4)^2 = \\
= Y^6 - 18 Y^5 + 133 Y^4 - 516 Y^3 + 1108 Y^2 - 1248 Y + X^2 - 2X + 577
\end{align*}


$G + \mathbb{I}(W) \in Ker(\bar{f}) \implies G(X, Y^2, X+Y) \in \mathbb{I}(V)$. Y por tanto $G=0$


Luego $\bar{f}$ es inyectiva.
