...
3. \todo{Cada ideal finitamente generado es principal.}
#+end_wording
...
** Primer apartado
*Si $A$ es un dominio de integridad (DI), ¿qué condición tiene que verificar $f(X)$ para que $\phi$ sea un isomorfismo?*
...
** Segundo apartado
*¿Qué ocurre cuando $A$ no es un DI?*
...
** Primer apartado
*Si $f(X) \in \mathbb{Z}[X]$ es irreducible, prueba que $f(X)$ es irreducible sobre $\mathbb{Q}$*
...
** Segundo apartado
*Si $f(X) \in \mathbb{Z}[X]$ es irreducible, entonces $F = \frac{\mathbb{Q}[X]}{(f(X))}$ es un cuerpo y tenemos inclusiones $\mathbb{Z} \subset \mathbb{Q} \subset F$.*
...
** Tercer apartado
*Si llamamos $x=X+(f(X))\in F$, prueba que $x$ es una raíz de $f(X)\in F[X]$*
...
** Primer apartado
*Prueba que $[\mathbb{Q}(\sqrt{2}, \sqrt{3}, \sqrt{5}): \mathbb{Q}] = 8$*
...
** Cuarto apartado
 *Encuentra elementos $\beta\in F$, de grado cuatro sobre $\mathbb{Q}$*
...
** Primer apartado
*Prueba que si $\alpha\in K$ es una raíz de $f(X)$, entonces $\{\alpha^2, \alpha^4, \alpha^{2n-1}\}$ son raíces de $f(X)$ en $K$*
...
** Segundo apartado
*Prueba que en general $\{\alpha,\alpha^2, \alpha^4, \alpha^{2n-1}\}$ no son todas las raíces de $f(X)$*
...
** Tercer apartado
*Prueba que si $\beta$ es una raíz primitiva de $K$, que es raíz de $f(X)$, entonces el grado de $f(X)$ es mayor o igual que $n$*
