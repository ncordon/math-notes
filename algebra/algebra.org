...
\[f(X(_1, \ldots X_n) = g(s_1, \ldots s_n)\]
...
6 se demuestra desde \ref{poly-simpl} haciendo $a_0 = f(0) = (-1)^n\prod_{i=1}^n \alpha_i$ y  $R(X^k, f) = a_n^k(\prod_{i=1}^n -\alpha_i)^k$ y sustituyendo. Para la segunda parte, basta aplciar el apartado 3.
...
LLamamos $f$ a $D(f)=a_n^{2n-2}\prod_{i<j}(\alpha_i-\alpha_j)^2$.
#+end_definition
