...
Claramente $V^\perp$ es sub vectorial de $H$ y además $V\subseteq (V^\perp)^\perp$. En $\mathbb{R}^n$, $V = (V^\perp)^\perp$, pero en espacios de Hilbert arbitrarios no tiene por qué cumplirse.
...
*** Un espacio ortogonal es cerrado en un Hilbert
...
*** Caracterización del ortogonal
Sea $w\in H$. Entonces:
\[w\in V^\perp \Leftrightarrow ||w|| = \inf_{v\in V}||w-v||\]

label:claim:ortogonal
**** Demostración
***** Hacia la derecha
...
*** La distancia a un Hilbert se materializa
...
Por tanto $\seq{v_n}{v} \in H$, y ello prueba la existencia. Veamos la unicidad: sea otro $\bar{v} \in H$ materializando la distancia:
...
Recordamos que la norma de operadores se define como $\opnorm{T}{H}{V} = \sup_{v\in V, \quad \norm{v}_V = 1} \norm{T(v)}_W$
